...
query=/data0/CannabisGenomics2021/Everybody/petah_pahka/HlPT1.fasta
...
for i in "${!assembly_paths[@]}"; do
    echo "Current assembly:" ${assembly_names[i]} 
...
if [[ -e ${assembly_paths[i]}.nhr ]]; then
    ...
    echo "Running tblastx for" ${assembly_names[i]}"..."

    tblastx -query $query -db ${assembly_paths[i]} -outfmt 6 | sort -k 7 -n > ${outdir}${assembly_names[i]}_HlPT1.tblastx
